...
// Validar fechas
$fechaInicio = $_POST["fechaInicioDevoluciones"] ?? Date("Y-m-d");
$fechaFin = $_POST["fechaFinDevoluciones"] ?? Date("Y-m-d");
...
$lector = $_POST["visitanteDevoluciones"] ?? 'todos';
$estado = $_POST["estadoDevoluciones"] ?? ''; // Asegúrate de usarlo si es necesario.
...
if ($estado != "todos") {
    $subFiltro .= " AND p.estado IN (?)";
    $placeholders[] = $estado; // Agrega el valor de $estado
    $confirmarEstado = true;
} else {
    $subFiltro .= " AND p.estado IN (2, 5)";
}

$filtro = "WHERE p.fecha_inicio BETWEEN DATE(?) AND DATE(?) $subFiltro ORDER BY p.estado";
$values = array_merge([$fechaInicio, $fechaFin], $placeholders);

// Obtener datos del reporte
try {
    $datos = $reporte->obtenerDatosParaReporte("devoluciones", $filtro, $values);
    ...
    /*function duplicateArray($array, $times) {
        if ($times <= 0) {
            return []; // Return an empty array if times is non-positive
        }
    ...
    $datos = duplicateArray($datos, 10);*/

    $html = '';

    if (count($datos) > 0) {
         // Calcular el resumen
    $totalDevoluciones = count($datos);
    ...
    if ($confirmarQueHayLector){
        $lectorName = " por <strong>" . $datos[0]["nombre"] . "</strong>, titular de la cédula <strong>" . $datos[0]["cedula"] . "</strong>";
    }

    $devolucionSingOplural = count($datos) > 1 ? "devoluciones realizadas" : "devolución realizada";

    $estadoDevolucion = "entre el";
    if ($confirmarEstado){
        $estadoDevolucion = $datos[0]["eprestamo"]  == 2 ? "a tiempo entre el" : "con retraso entre el";
    }
    

    // Resumen en la primera página
    $resumen = "<h3 style='text-align:center;'>Resumen de Devoluciones</h3>";
    $resumen .= "<p style='text-align:justify;'>Este reporte muestra un total de <strong>$totalDevoluciones</strong> $devolucionSingOplural $estadoDevolucion <strong>$fechaInicioFormateada</strong> y el <strong>$fechaFinFormateada</strong>$lectorName. A continuación, se detallan las devoluciones registradas en el sistema:</p>";
    $pdf->writeHTML($resumen, true, false, true, false, '');

    // Contenido del reporte
    $html .= '<h3 style="text-align:center;">Detalles de Devoluciones</h3>';
    ...
    foreach ($datos as $index => $prestamo) {
        if ($recordCount % $recordsPerPage == 0) {
            ...
            if ($recordCount > 0) {
                ...
                $html = '<h3 style="text-align:center;">Detalles de Devoluciones (continuación)</h3>';
            }
                ...
                <thead>
                    <tr style="background-color:#f2f2f2;">
                        ...
                        if (!$confirmarQueHayLector){
                            ...
                            $html .='
                            <th style="border:0.5px solid #ddd; text-align:center; width: 130px;">Título</th>';
                        }
                        ...
                        <th style="border:0.5px solid #ddd; text-align:center;">Estado</th>
                    </tr>
                </thead>
                <tbody>';
        }
        ...
        $html .= '<tr style="background-color:' . $bgColor . ';">
            ...
            if (!$confirmarQueHayLector){
                $html .= '
                <td style="border:0.5px solid #ddd; white-space: nowrap; width: 100px;">' . nl2br(htmlspecialchars($prestamo["titulo"])) . '</td>';
            } else {
                $html .= '
                <td style="border:0.5px solid #ddd; white-space: nowrap; width: 130px;">' . nl2br(htmlspecialchars($prestamo["titulo"])) . '</td>';
            }
            ...
            if (!$confirmarQueHayLector){
                $html .= '
            <td style="border:0.5px solid #ddd;">' . $prestamo['nombre'] . '</td>
            ...
            <td style="border:0.5px solid #ddd; text-align:center; font-weight:bold; color:' . ($prestamo['estado'] == 'devuelto' ? '#00FF00' : '#FF0000') . ';">' . ucfirst(strtolower($prestamo['estado'])) . '</td>
        </tr>';
    ...
    $pdf->writeHTML('<p style="text-align:right; font-weight:bold;">Total de Devoluciones: ' . count($datos) . '</p>', true, false, true, false, '');
